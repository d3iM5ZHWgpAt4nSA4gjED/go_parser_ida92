...
def find_func_by_name(func_name):
    for segea in idautils.Segments():
        for funcea in idautils.Functions(segea, idc.get_segm_end(segea)):
            if func_name == ic.get_func_name(funcea):
...
def read_mem(addr, forced_addr_sz=None, read_only=False):
    ...
    if not read_only: # Set bytes to undefined firstly
        if forced_addr_sz:
            ic.del_items(addr, forced_addr_sz, ic.DELIT_SIMPLE)
        else:
            ic.del_items(addr, ADDR_SZ, ic.DELIT_SIMPLE)
        ic.auto_wait()

    if forced_addr_sz == 2:
        if not read_only:
            idc.create_data(addr, ic.FF_WORD, 2, ic.BADADDR)
            ic.auto_wait()
        value = idc.get_wide_word(addr) & 0xFFFF
        return 0 if value == ic.BADADDR else value
    if forced_addr_sz == 4 or ADDR_SZ == 4:
        if not read_only:
            idc.create_data(addr,ic.FF_DWORD, 4, ic.BADADDR)
            ic.auto_wait()
    ...
    if forced_addr_sz == 8 or ADDR_SZ == 8:
        if not read_only:
            idc.create_data(addr, ic.FF_QWORD, 8, ic.BADADDR)
            ic.auto_wait()
...
def get_goroot():
    ...
    for cb_idx in ret_cbs:
        if idc.get_operand_type(goroot_flowchart[cb_idx].start_ea, 0) == 1:
            ...
            curr_addr = ic.find_code(curr_addr, ic.SEARCH_DOWN)
            ...
            if goroot_path_str is None or len(goroot_path_str)==0:
                ...
                return ""
            ic.create_strlit(goroot_path_addr, goroot_path_addr+goroot_path_len)
            ic.auto_wait()
            break
...
def find_ret_cb(flow_chart):
    ...
    for idx in range(flow_chart.size):
        if flow_chart[idx].type == ic.FCB_RET:
    ...
    for fc_idx in range(schedinit_flowchart.size):
        ...
        if idc.print_insn_mnem(fc.start_ea) == "mov" and idc.get_operand_type(fc.start_ea, 0) == 2 \
            ...
            if possible_goversion_len >=5 and possible_goversion_len < 10:
                if ic.create_strlit(possible_goversion_str_addr, possible_goversion_str_addr + possible_goversion_len):
                    ic.auto_wait()
