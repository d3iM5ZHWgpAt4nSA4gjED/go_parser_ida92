...
def is_string_patt(addr):
    ...
    if idc.print_operand(addr, 0) in VALID_REGS \
        ...
        addr_2 = ic.find_code(addr, ic.SEARCH_DOWN)
        try:
        ...
        if idc.print_insn_mnem(addr_2) == 'mov' and dest_reg in VALID_DEST \
            ...
            addr_3 = ic.find_code(addr_2, ic.SEARCH_DOWN)
...
def parse_str_ptr(addr):
    ...
    if str_len > 1:
        if ic.create_strlit(str_ptr, str_ptr+str_len):
            ic.auto_wait()
            if opnd_diff > 0:
                idc.set_cmt(addr, "length: %d" % str_len,0)
                ic.add_dref(addr_2, str_ptr, idaapi.dr_O)
            else:
                idc.set_cmt(addr_2, "length: %d" % str_len,0)
                ic.add_dref(addr, str_ptr, idaapi.dr_O)
            ic.auto_wait()
            return True
...
def create_string(addr, string_len):
    ...
    if idc.get_str_type(addr) is not None and idc.get_strlit_contents(addr) is not None and len(idc.get_strlit_contents(addr)) != string_len:
        ...
        ic.del_items(addr, ic.DELIT_SIMPLE,string_len)
        ic.auto_wait()

    if idc.get_strlit_contents(addr) is None and ic.create_strlit(addr, addr + string_len):
        ic.auto_wait()
        return True
    else:
        ...
        ic.del_items(addr,ic.DELIT_SIMPLE, string_len)
        ic.auto_wait()
        if ic.create_strlit(addr, addr + string_len):
            ic.auto_wait()
            return True
...
def create_offset(addr):
    if ic.op_plain_offset(addr, 1, 0):
        return True
    else:
    ...
    #for addr in idautils.Functions(text_seg.start_ea, text_seg.end_ea):
            ...
            while addr <= end_addr:
                if parse_str_ptr(addr):
                    strings_added += 1
                    addr = ic.find_code(addr, ic.SEARCH_DOWN)
                    ...
                    addr_3 = ic.find_code(ic.find_code(addr, ic.SEARCH_DOWN), ic.SEARCH_DOWN)
                    ...
                    addr = ic.find_code(addr_3, ic.SEARCH_DOWN)
                else:
                    addr = ic.find_code(addr, ic.SEARCH_DOWN)
