...
class TypesParser():
    ...
    def _parse_extra_types(self):
        ...
        if runtime_newobject_addr == ic.BADADDR:
        ...
        for xrf in idautils.XrefsTo(runtime_newobject_addr):
            ...
            if target_type_addr == ic.BADADDR:
    ...
    def parse_type(self, type_addr=ic.BADADDR, depth=1):
        if type_addr == 0 or type_addr == ic.BADADDR:
            return None
...
class RType():
    ...
    def __init__(self, addr, firstmoduledata, type_parser):
        ...
        self.gcdata = None
        self.name_off = 0
        self.name_addr = ic.BADADDR
        self.name_obj = None
        self.name = ""
        self.ptrtothis = None
        self.ptrtothis_off = 0
        self.ptrtothis_addr = ic.BADADDR
    ...
    def parse(self):
        ...
        if len(self.name) > 0:
            idc.set_name(self.addr, self.name, flags=ic.SN_FORCE)
            ic.auto_wait()

        # parse type pointer
        if self.ptrtothis_off > 0 and self.ptrtothis_addr != ic.BADADDR:
...
class Name():
    ...
    def parse(self, has_star_prefix):
        ...
        if self.moddata.magic_number == common.MAGIC_112 or self.moddata == common.MAGIC_116:
            ic.create_strlit(self.addr + 3, self.addr + 3 + self.len)
        else:
            ic.create_strlit(self.addr + 1 + self.namestr_off, self.addr + 1 + self.namestr_off + self.len)
        ic.auto_wait()

        if self.is_followed_by_tag:
            if self.moddata.magic_number == common.MAGIC_112 or self.moddata == common.MAGIC_116:
                ic.create_strlit(self.addr + 1 + self.namestr_off + self.tag_off_sz, \
                ...
                idc.set_cmt(self.addr + 1 + self.namestr_off + self.tag_off_sz, f"tag of @ {self.addr:#x}", 0)
                 ...
                 ic.create_strlit(self.addr + 3 + self.len + 2, self.addr + 3 + self.len + 2 + self.tag_len)
            ic.auto_wait()
...
class PtrType():
    ...
    def __init__(self, addr, type_parser, rtype):
        ...
        self.target_type_addr = ic.BADADDR
...
class StructType():
    ...
    def parse(self):
        ...
        if self.pkg_path_addr > 0 and self.pkg_path_addr != ic.BADADDR:
        ...
        if len(self.rtype.name) > 0 and fields_cnt > 0:
            ...
            idc.set_name(fields_start_addr, f"{self.rtype.name}_fields", flags=ic.SN_FORCE)
            ic.auto_wait()
...
class StructFiled():
    ...
    def __init__(self, addr, type_parser):
        self.addr = addr
        self.type_parser = type_parser
        self.name_obj_addr = ic.BADADDR
        self.name_obj = None
        self.name = ""
        self.rtype_addr = ic.BADADDR
        self.rtype = None
        self.offset = 0
    ...
    def parse(self):
        ...
        if self.name_obj_addr == 0 or self.name_obj_addr == ic.BADADDR:
        ...
        if self.rtype_addr == 0 or self.rtype_addr == ic.BADADDR:
...
class ArrayType():
    ...
    def parse(self):
        ...
        idc.set_name(self.addr, f"{self.elem_type.name}_array", flags=ic.SN_FORCE)
        ic.auto_wait()
...
class SliceType():
    ...
    def parse(self):
        ...
        idc.set_name(self.addr, f"{self.elem_rtype.name}_slice", flags=ic.SN_FORCE)
        ic.auto_wait()
...
class InterfaceType():
    ...
    def parse(self):
        ...
        if len(self.rtype.name) > 0:
            idc.set_name(methods_start_addr, f"{self.rtype.name}_methods", flags=ic.SN_FORCE)
            ic.auto_wait()
...
class IMethodType():
    ...
    def parse(self):
        ...
        if type_off > 0 and type_addr != ic.BADADDR:
        ...
        if name_off > 0 and name_off != ic.BADADDR:
...
class MethodType():
    ...
    def __init__(self, addr, type_parser):
        ...
        self.name_obj = None
        self.name = ""
        self.mtype_addr = ic.BADADDR
        self.mtype = None
        self.ifn_addr = ic.BADADDR
        self.ifn_off = 0
        self.tfn_addr = ic.BADADDR
        self.tfn_off = 0
        self.size = 4*4
    ...
    def parse(self):
        ...
        ifn_name = ic.get_func_name(self.ifn_addr)
        ...
        tfn_name = ic.get_func_name(self.tfn_addr)
...
def _get_target_addr_arm32(xrf_addr):
    ...
    for i in range(8): # Search back 8 insns at most
        ...
        if target_opnd_type == 0x1 or target_opnd_value == 0x0: # Reg X0
            ...
            if target_type_addr_ptr > 0 and target_type_addr_ptr != ic.BADADDR:
