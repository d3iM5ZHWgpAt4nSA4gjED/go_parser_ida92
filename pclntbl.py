...
class Pclntbl():
    ...
    def __init__(self, start_addr, magic_number):
        ...
        self.func_tbl_addr    = ic.BADADDR
        self.func_tbl_sz      = 0 # Size of whole function table
        self.pctbl_addr       = ic.BADADDR
        self.cutbl_addr       = ic.BADADDR
        self.text_sect_addr   = ic.BADADDR
        #self.func_sym_tbl    = dict() # pc -> FunctionSymbolTableEntry
        self.end_pc           = 0
        self.srcfile_tbl_addr = ic.BADADDR
    ...
    def parse_hdr(self):
        ...
        if self.magic_number == common.MAGIC_112 \
            ...
            raise RuntimeError("Invalid pclntbl header magic number!")
            #raise Exception("Invalid pclntbl header magic number!")

        idc.create_data(self.start_addr, ic.FF_DWORD, 4, ic.BADADDR)
        idc.set_cmt(self.start_addr, "Magic Number", 0)
        idc.set_name(self.start_addr, "runtime_symtab", flags=ic.SN_FORCE)
        ic.auto_wait()

        if idc.get_wide_word(self.start_addr + 4) & 0xFFFF != 0:
            raise Exception("Invalid pclntbl header")
        idc.create_data(self.start_addr + 4,ic.FF_WORD, 2, ic.BADADDR)
        ...
        if self.magic_number == common.MAGIC_112:
            ...
            idc.set_name(funcs_entry, "funcs_entry", flags=ic.SN_FORCE)
            idc.set_name(self.func_tbl_addr, "pc0", flags=ic.SN_FORCE)
            ic.auto_wait()
        if self.magic_number == common.MAGIC_116:
            ...
            idc.set_name(self.srcfile_tbl_addr, "runtime_filetab", flags=ic.SN_FORCE)
            ic.auto_wait()
            ...
            idc.set_name(self.func_tbl_addr, "runtime_functab", flags=ic.SN_FORCE)
            ic.auto_wait()
    ...
    def parse_funcs_112(self):
        ...
        for func_idx in range(self.func_num):
            ...
            if not ic.get_func_name(func_addr):
                ic.del_items(func_addr, ic.DELIT_EXPAND)
                ic.auto_wait()
    ...
    def parse_funcs_118(self):
        ...
        for func_idx in range(self.func_num):
            ...
            if not ic.get_func_name(func_entry_addr):
                ic.del_items(func_entry_addr, ic.DELIT_EXPAND)
                ic.auto_wait()
    ...
    def parse_srcfile_112(self):
        ...
        for idx in range(self.srcfile_num):
            ...
            ic.create_strlit(srcfile_addr, srcfile_addr + len(srcfile_path) + 1)
            ic.auto_wait()
            idc.set_cmt((idx+1) * 4 + self.srcfile_tbl_addr, "", 0)
            ic.add_dref((idx+1) * 4 + self.srcfile_tbl_addr, srcfile_addr, idaapi.dr_O)
            ic.auto_wait()
        common._info("--------------------------------------------------------------------------------------")
    ...
    def parse_srcfile_latest(self):
        ...
        for idx in range(self.srcfile_num):
            ...
            ic.create_strlit(curr_srcfilename_addr, curr_srcfilename_addr + len(srcfile_path) + 1)
            ic.auto_wait()
...
class FuncStruct():
    ...
    def parse(self, is_test=False):
        ...
        # Make name string
        if len(self.name) > 0:
            if ic.create_strlit(name_addr, name_addr + len(raw_name_str) + 1):
                ic.auto_wait()
        ...
        if len(self.name) > 0 and real_func_addr is not None:
            if idc.set_name(real_func_addr.start_ea, self.name, flags=ic.SN_FORCE):
                ic.auto_wait()
            ...
            while data_ref != ic.BADADDR:
                if idc.get_segm_name(data_ref) in possible_seg:
                    ...
                    if 'off_' in idc.get_name(data_ref):
                        if idc.set_name(data_ref, f"{name}_ptr", flags=ic.SN_FORCE):
                            ic.auto_wait()
                            renamed += 1
                        else:
